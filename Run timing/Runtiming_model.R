...
output$yrange <- renderUI({
  dates <- as.Date(data.0()$Date,"%d-%b")
  fyear <- min(dates)       # First brood year 
  lyear <- max(dates)       # Last brood year
  #  Slider input UI 
  dateRangeInput("sryears", "Select date range:",
                 start = fyear,
                 end   = lyear,
                 min    = fyear,
                 max    = lyear,
...
data <- reactive({
  ...
  dat <- dat[which(dates>=input$sryears[1]&dates<=input$sryears[2]), ]
  return(dat)
  })
# output table -----------------------------------------------------------------
