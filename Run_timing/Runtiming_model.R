...
numericInput(inputId='n.sigma','Sigma',value=50,min=1,step = 10),
...
output$fpage = renderUI({
  mn <- length(years())%/%4
  numericInput("pf", "page", value=1,min=1,max=mn,step=1)
  })
#Bayesian trace-density plot figure page
output$lpage = renderUI({
  mn <- length(years())%/%2
  numericInput("p1", "page", value=1,min=1,max=mn,step=1)
})
...
inits <- reactive({
  ...
  for (j in 1:nyrs){
    ...
    b[j]<-   1/sqrt(log(sum(x[,j+1],na.rm=TRUE)))
  }
...
#-----------------------------------------------------------------------
      ...
      #       lambda[j,i] <- theta[j,i]      #  Negative-binomial as 
...
output <- jags(data=append(datnew,hyper),inits=initss,parameters.to.save=parameters, model.file= jagmodel,
