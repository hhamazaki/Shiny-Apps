...
#-----------------------------------------------------------------------
# bsummmary------- Bootstrap summary ------------------------------------ 
output$Txt_sum.b <- renderPrint({
  print(summary(boot.SR()[,c(5,2,1,3,6:10)]),digits=c(3,10,3,0,0,0,0,0,0))
  })
# Txt_quantile.b ------ Bootstrap Qunile-------------------------------------
output$Txt_quantile.b <- renderPrint({
  print(apply(boot.SR()[,c(5,2,1,3,6:10)],2,
      function(x) quantile(x,probs=c(0.025,0.975),na.rm=TRUE)),
      digits=c(3,10,3,3,0,0,0,0,0,0))
  })

# Plt_hist.b ----------- Plot histogram  --------------------------------------------
output$Plt_hist.b <- renderPlot({
  par(mfrow=c(3,3),mar = c(1.75,1.5,1.5,1.75),xaxs='i',yaxs='i',bty='l')
  plot(density(boot.SR()$alpha),main='Ricker alpha',xlab='',ylab='')
  plot(density(boot.SR()$beta),main='Ricker beta',xlab='',ylab='')
  plot(density(boot.SR()$phi),main='AR1 phi',xlab='',ylab='')  
  plot(density(boot.SR()$Seq), main='SEQ',xlab='',ylab='')
  plot(density(boot.SR()$Smsy),main='Smsy',xlab='',ylab='')
  plot(density(boot.SR()$Smax), main='Smax',xlab='',ylab='')
  plot(density(boot.SR()$MSY,na.rm=T), main='MSY',xlab='',ylab='',xlim=c(0,quantile(boot.SR()$MSY,0.99)))
  plot(density(boot.SR()$Rmax,na.rm=T), main='Rmax',xlab='',ylab='',xlim=c(0,quantile(boot.SR()$Rmax,0.99)))
  })
...
# For each simulation, assign 1 if valu of M.rep is >  greater than miimum % M.rep
# Assign 0  0 if not
  for(j in 1:nrows){
