...
#-----------------------------------------------------------------------
  tabPanel("SR Analyses",
    sidebarPanel(width = 3,
      textInput(inputId='caption',label='Figure Caption Title',value=''),  
   ...
   navbarMenu("Escapement Goal Analyses",
      ...
      tabPanel("Smax Goal Analyses",
       ...
       tabPanel("Yield & Recruit Goal Analyses",
         ...
         numericInput("y1", "Min Yied", value=100000,min=0, step=10000),
...
SR.out <- reactive({
  ...
  RSmsy <- Smsy*exp(ln.alpha-beta*Smsy)
  MSY <- RSmsy-Smsy
  Smax <- 1/beta
...
# Prediction model s range  
  s <- seq(0,max(1.2*par$Seq,data()$S), length.out =101)
# Predicttion 
...
# Smsy Goal BEG Out
 SA.BEG <- reactive({
   ...
   mp.2 <- input$p2/100
   u <- as.numeric(input$ui)
   BEG.1 <- round(b.SA()$BEG.1/u,3)
   BEG.2 <- round(b.SA()$BEG.2/u,3)
...
boot.Yldp <- reactive({
  ...
  legend('topright',paste(mp,'% CI'),fill='grey',border=0,box.lty=0)    
  out <-recordPlot()
  return(out)
})
...
# Add CI    
  ...
  with(SRp,polygon(c(s,rev(s)),c(lwr,rev(upr)),col=tcol('grey',50),border=NA))
  }
# Add Years
...
# Add CI    
  if(input$show.int==TRUE){
  with(SRp,polygon(c(s,rev(s)),c(lwr-s,rev(upr-s)),col=tcol('grey',50),border=NA))
   }
# Add Years
  if(input$show.points==TRUE) {
...
output$byp <- renderPlot({
   ...
   lines(s,SRp()$prof.Ypi,lty=2)
   abline(h = ypg,lwd=2,col=2)
   legend('topright',c('Mean Yield','Annual Yeald'),lty=c(1,2),box.lty=0) 
  })  
# Print Optimum Mean Yield Proflie Goal Range  
output$bypt <-renderText({
   u <- as.numeric(input$ui)
   yg <- input$y1
   ypg <- input$y1p
  BEG.p <- round(b.YApg()/u,3)
   tx1 <- paste0('Achieving Minimum Mean Yield ',yg,', >',ypg,'%') 
   bg1 <- paste('Escapement Goal Range:',BEG.p[1],'-',BEG.p[2])
   s <- SRp()$s
   Ayp <- SRp()$prof.Ypi
   BEGs <- round(s[sign(Ayp-ypg/100)==1]/u,3)    
   tx2 <- paste0('Achieving Minimum Annal Yield ',yg,', >',ypg,'%')    
   bg2 <- paste('Escapement Goal Range:',min(BEGs),'-',max(BEGs))
   paste(tx1,bg1,tx2,bg2,sep='\n')   
 })
